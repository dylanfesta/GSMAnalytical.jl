...
function meanlog_px(xs::Matrix{R},gsm::GSM{RayleighMixer{R},R}) where R
  l=0.0
  for x in eachcol(xs)
    l+= log(p_x(x,gsm))
  end
...
"""
    function loglik_data(x::AbstractMatrix{R},gsm::GSM) where R

Log-likelyhood test of datapoint for a given model.
Returns the averaged log-probability that the data has been
generated by a certain gsm model.
"""
function loglik_data(x::AbstractMatrix{R},gsm) where R
  return meanlog_px(x,gsm)
end
